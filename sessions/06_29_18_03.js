...
beat = which => length => active => list => list.map((d,i) => active.includes(i % length) ? which : d === which ? 0 : d)
notes = bd(8)([0])(notes)
stop()
const snap = scale => list => list.map(d => scale.includes(d % 12) ? d : d + 1)
snap2 = scale => x => scale.includes(x) ? x : snap(scale)(x + 1)
let fitScale = scale => list => list.map(snap(scale))
notes = [43, 43,43, 48]
snap2 = scale => x => scale.includes(x % 12) ? x : snap(scale)(x + 1)
start()
tempo += 100
let aeolian = [0,2,3,5,7,8,10]
fitScale = scale => list => list.map(snap2(scale))
notes = fitScale(aeolian)(notes)
notes = [44, 46, 48, 50]
notes = fitScale(aeolian)(notes)
snap2(aeolian)([44])
snap2(aeolian)([44})
snap2(aeolian)([44])
snap2 = scale => x => scale.includes(x % 12) ? x : snap2(scale)(x + 1)
snap2(aeolian)([44])
snap2(aeolian)([45])
snap2 = scale => x => scale.includes(x % 12) ? x : snap2(scale)(parseInt(x + 1))
snap2(aeolian)([45])
